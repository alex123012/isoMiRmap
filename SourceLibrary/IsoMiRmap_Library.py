...
def open_read_gz(arg_fn, MD5_check):
    ...
    if ".gz" in str(arg_fn):
        try:
            ...
            print(
                "Error, exiting: file " + str(arg_fn) + " could not be opened",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
    ...
    if MD5_check != None:
        ...
        if MD5_check != md5check:
            print(
                "Error, exiting: md5sum defined in tables.cfg does not match for {}: {} vs {}".format(
                    str(arg_fn), MD5_check, md5check
                ),
                file=sys.stderr,
            )
            sys.exit(1)
...
def process_args():
    ...
    parser.add_argument(
        ...
        "--mappingbundle",
        type=str,
        default=DEFAULT_MAPPINGTABLES,
        dest="mappingtables",
        help='If not specified, “mappingbundle” will be set to "'
        + DEFAULT_MAPPINGTABLES
        + '". This is the'
    ...
    parser.add_argument(
        ...
        "--outputprefix",
        type=str,
        default=DEFAULT_OUTPREFIX,
        dest="outputprefix",
        help='If not specified, “outputprefix” will be set to "'
        + DEFAULT_OUTPREFIX
        + '" and all'
    ...
    parser.add_argument(
        ...
        choices=["n", "c"],
        dest="enableediting",
        help="This is an optional argument. 'n' - for all editing events,"
        " 'c' - for canonical editing events (C-to-U and A-to-I)",
    )
    ...
    if not os.path.exists(str(Path(args.mappingtables) / "tables.cfg")):
        print(
            "Error, exiting: Config file tables.cfg not found in directory: "
            + args.mappingtables,
            file=sys.stderr,
        )
        sys.exit(1)
...
def load_configtable(fn_configtable):
    ...
    for line in open_lookup:
        read_line = line.split("#")[0].strip()
        # remove any comments from line if they exist
    ...
    if mappingtables_name == None:
        print(
            "Error, exiting: MAPPINGTABLES_NAME not set in tables.cfg", file=sys.stderr
        )
        sys.exit(1)
    if mappingtables_rev == None:
        print(
            "Error, exiting: MAPPINGTABLES_REV not set in tables.cfg", file=sys.stderr
        )
        sys.exit(1)
    if mappingtables_description == None:
        print(
            "Error, exiting: MAPPINGTABLES_DESCRIPTION not set in tables.cfg",
            file=sys.stderr,
        )
        sys.exit(1)
...
def load_splicedfasta(sequence):
    ...
    for line in open_sequence:
        if header:
            ...
            if header[0] != ">":
                print(
                    "Error, exiting: Expected FASTA header but received "
                    + header
                    + " instead.",
                    file=sys.stderr,
                )
            ...
            if not ALPHANUMERIC.match(hairpinname):
                print(
                    "Error, Only characters [&-.|+_A-Za-z0-9] allowed in FASTA label but received "
                    + header
            ...
            if not VALID.match(sequence):
                print(
                    "Error, exiting: Only characters [ATGCN] allowed in FASTA sequence but received "
                    + sequence
                    + " instead",
                    file=sys.stderr,
                )
...
def load_lookup_table(lookuptable):
    ...
    for line in open_lookup:
        ...
        if min_fragment_len == None or len(line_split[0]) < min_fragment_len:
            min_fragment_len = len(line_split[0])
            # keep track of the minimum fragment size inside the lookup tasble
...
def add_annotations(array_addto, isomirfull, isomircut, hash_hairpins):
    ...
    onematch = False
    # for error checking, make sure isomircut is at least found once or else something is wrong
    ...
    for hairpinname in hash_hairpins.keys():
        ...
        while result != -1:
            onematch = True
            matchprev = hairpinname + "@" + str(result + 1) + "."
            # this is a match key to see if it was found in the same hairpin at the same start location

            addme = True
            # add the hairpin location if it hasn't been added already (potentially between T-trimming)
            ...
            if (
                addme == True
            ):  # add outside of the iterator above since adding while searching may cause issues
                mod3p = ""
    ...
    if not onematch:
        print(
            "Error, exiting: no annotations in sequence file "
            + fn_splicedsequences
            + " found for isomiR "
            + isomircut,
            file=sys.stderr,
...
def get_meta_intersects(metaannotations_hash, annotation_list):
    ...
    for annotationentry in annotation_list:
        ...
        postmod = ""
        # lets carry over the hairpin post-modification annotations to any mature intersect annotations
        ...
        if keylookup in metaannotations_hash:
            ...
            for coordinate_intersects in metaannotations_hash[keylookup]:
                ...
                if (
                    intersect_start <= isomir_coord_start <= intersect_end
                    or intersect_start <= isomir_coord_end <= intersect_end
                    or (
                        isomir_coord_start < intersect_start
                        and isomir_coord_end > intersect_end
                    )
                ):
...
def create_output(
    read_hash,
    annotation_hash,
    otherannotations_hash,
    largest_postmod,
    total_frags_in_file,
    seqcategory,
    metaannotations_hash,
    output_prefix,
    customrpm,
    edited_to_ref=None,
):
    ...
    ret_file_name = (
        output_prefix + "-" + scriptversion + "-" + seqcategory + ".expression.txt"
    )
    ...
    expression_file.write(
        ...
        + mappingtables_rev
        + "): "
        + mappingtables_description
        + "\n"
    )
    write_argsheader(expression_file, False)
    expression_file.write(
        "## RPM* - Using [{}] as denominator - total number of reads in this output file.\n"
    ...
    expression_file.write(
        "License Plate\tIsomiR sequence\tType\tUnnormalized read counts\tRPM*\tRPM**\tRPM***\t"
    ...
    filename = (
        output_prefix + "-" + scriptversion + "-" + seqcategory + ".expression.html"
    )
    ...
    html_file.write(
        "<html><head><title>{} expression</title></head>\n".format(seqcategory)
    )
    html_file.write(
        "<style> tr:nth-of-type(odd) { background-color:#ccc; } body { font-size: 18px; } table"
    ...
    html_file.write(
        '<table style="width=100%; white-space: nowrap;"><tr><td><b>License Plate</b><br />\n(sequence derived)</td>'
        ...
        if read_hash[key] != 0:
            ...
            if fn_otherannotations != None:
                try:
                    ...
                    if modsize == 0:
                        otherannotations = otherannotations_hash[
                            key if edited_to_ref is None else edited_to_ref[key]
                        ]
                    else:
                        otherannotations = otherannotations_hash[
                            key[:-modsize]
                            if edited_to_ref is None
                            else edited_to_ref[key][:-modsize]
                        ]  # get repeat masker annotations from the smallest possible templated sequence of the isomiR

                except KeyError:
                    print(
                        "Using OtherAnnotations but OtherAnnotation for "
                        + key
            ...
            metaannotations = "na"
            metaarray = []
            arrayofarrays = get_meta_intersects(
                metaannotations_hash, annotation_hash[key]
            )
            ...
            metaannotations_html = ",<br />".join(
                list(map(lambda x: "[" + x + "]", metaarray))
            )
    ...
    filename = (
        output_prefix + "-" + scriptversion + "-" + seqcategory + ".countsmeta.txt"
    )
    print("Creating output file " + filename)
    ...
    countsmeta.write(
        "Total reads in -f input file\tTotal unnormalized reads in "
        + seqcategory
        + "\tPercent\n"
    )
    countsmeta.write(
...
def create_snp_output(
    ...
    expression_file.write(
        "License Plate\tIsomiR sequence\tUnnormalized read counts\t"
        "RPM*\tRPM**\tRPM***\t"
    ...
    for key in read_hash:
        if read_hash[key] != 0:
            ...
            for convert in re_convert:
                license_convert = license_convert.replace(
                    convert, MINT.convert(convert, True, "iso"), 1
                )
                # only replace first instance just incase
...
def create_gff3_output(outputprefix, args):
    ...
    if "--meta" in args:
        gff3_filename = (
            ...
            + scriptversion
            + "-"
            + DEFAULT_FRAGMENTTYPE
            + "s."
            + args[args.index("--meta") + 1]
            + ".gff3"
        )
...
def start_generation():
    ...
    fastfrag_hairpin_edited_annotations = {}
    # keep track of the largest pod modification recorded for a sequence
    fastfrag_largestPostMod = {}
    fastfrag_edited_largestPostMod = {}
    ...
    isomir_annotations = {}
    if fn_otherannotations != None:
        isomir_annotations = load_otherannotations(
            Path(args.mappingtables) / fn_otherannotations
        )
    ...
    metacoordinates = {}
    metacoordinates = load_metacoordinates(
        Path(args.mappingtables) / fn_metacoordinates
    )
    ...
    fastfrag_exclusive_edited_counts = {}
    fastfrag_notexclusive_edited_counts = {}
    fastfrag_edited_to_ref = {}
    ...
    stat_totalfragmentreads_edited_exclusive = 0
    stat_totalfragmentreads_edited_notexclusive = 0
    ...
    for line in trimmedfastqfile:
        ...
        if not VALID.match(line_seq):
            print(
                "Error, exiting: Unrecognized character in fastq sequence "
                + line_seq
                + ", only [ATCGN]'s allowed",
                file=sys.stderr,
            )
            sys.exit(1)
        else:
    ...
    for line_seq, seq_count in fastfrag_all_counts.items():
        ...
        stretch3p = ""
        # map stretches of A/T/C/G 3p post-modifications
        foundexact = False
        ...
        while len(potentialmod) >= min_fragment_len:
            ...
            if is_exclusive and is_notexclusive:
                print(
                    "Error, exiting: Fragment "
                    + potentialmod
                    + " should not exist in both the exclusive and ambiguous buckets",
                    file=sys.stderr,
                )
                sys.exit(1)
            ...
            # if it's in the lookup table, regardless of exclusivity
            if is_exclusive or is_notexclusive:
                foundexact = True
                fastfrag_hairpin_annotations[line_seq] = []
                fastfrag_largestPostMod[line_seq] = 0

                largestmod = add_annotations(
                    fastfrag_hairpin_annotations[line_seq],
                    line_seq,
                    potentialmod,
                    hairpinseq,
                ...
                break
                # once a best hit has been found, stop
        ...
        if not foundexact:  # lets look in SNP table if not found exactly
            ...
            if args.enableediting is not None:
                if not MINT.is_sequence(line_seq):
                    continue

                seq_len = len(line_seq)
                if args.enableediting == "c":
                    edited_seqs = [
                        line_seq[:i] + canonical_edit(line_seq[i]) + suff
                        for i in range(seq_len)
                        if (i + 1 < seq_len and (suff := line_seq[i + 1 :]))
                        or (suff := "")
                    ]

                elif args.enableediting == "n":
                    edited_seqs = [
                        line_seq[:i] + n + suff
                        for n in ["A", "T", "C", "G"]
                        for i in range(seq_len)
                ...
                edited_seqs = list(sorted(set(edited_seqs)))
                for ed_seq in edited_seqs:
                    potentialmod_edited = ed_seq
                    stretch3p_edited = ""
                    # map stretches of A/T/C/G 3p post-modifications

                    while len(potentialmod_edited) >= min_fragment_len:
                        is_exclusive = potentialmod_edited in fastfrag_exclusive
                        is_notexclusive = potentialmod_edited in fastfrag_notexclusive
                        if is_exclusive and is_notexclusive:
                            print(
                                "Error, exiting: Fragment "
                                + potentialmod_edited
                        ...
                        if is_exclusive:
                            stat_totalfragmentreads_edited_exclusive += seq_count
                            fastfrag_exclusive_edited_counts[line_seq] = seq_count
                        elif is_notexclusive:
                            stat_totalfragmentreads_edited_notexclusive += seq_count
                            fastfrag_notexclusive_edited_counts[line_seq] = seq_count

                        # if it's in the lookup table, regardless of exclusivity
                        if is_exclusive or is_notexclusive:
                            fastfrag_hairpin_edited_annotations[line_seq] = []
                            fastfrag_edited_largestPostMod[line_seq] = 0
                            ...
                            # keep track of largest post-modification found for a given sequence
                            if largestmod != 0:
                                fastfrag_edited_largestPostMod[line_seq] = largestmod

                            fastfrag_edited_to_ref[line_seq] = ed_seq
                            break
                            # once a best hit has been found, stop

                        if stretch3p_edited == "":
                            stretch3p_edited = potentialmod_edited[-1:]

                        if (not (potentialmod_edited[-1:] == stretch3p_edited)) or (
                            stretch3p_edited == "N"
                        ):
                            break
                        # remove last character
                        potentialmod_edited = potentialmod_edited[:-1]

    trimmedfastqfile.close()

    # Create output files for both the exclusive and non-exclusive fragments
    print("Generating output:")
    exclusive_file = create_output(
        fastfrag_exclusive_counts,
        fastfrag_hairpin_annotations,
        isomir_annotations,
        fastfrag_largestPostMod,
    ...
    exclusive_edited_file = create_output(
        fastfrag_exclusive_edited_counts,
        fastfrag_hairpin_edited_annotations,
        isomir_annotations,
        fastfrag_edited_largestPostMod,
        stat_totalfragmentreads_edited_exclusive,
        "exclusive-" + DEFAULT_FRAGMENTTYPE + "s",
        metacoordinates,
        args.outputprefix + "_edited",
        args.customrpm,
        fastfrag_edited_to_ref,
    )
    ambiguous_edited_file = create_output(
        fastfrag_notexclusive_edited_counts,
        fastfrag_hairpin_edited_annotations,
        isomir_annotations,
        fastfrag_edited_largestPostMod,
        stat_totalfragmentreads_edited_notexclusive,
        "ambiguous-" + DEFAULT_FRAGMENTTYPE + "s",
        metacoordinates,
        args.outputprefix + "_edited",
        args.customrpm,
    ...
    if (
        fn_snptable != None
    ):  # only create SNP output if the snp table is in the mappingbundle
        create_snp_output(
            fastfrag_snps_counts,
            isomir_snps,
    ...
    create_gff3_output(
        args.outputprefix + "_edited",
        [
            exclusive_edited_file,
            ambiguous_edited_file,
            "--meta",
            "miRCarta",
            "--mt_name",
            mappingtables_name,
...
EDITING_DICT = {"A": "G", "C": "U"}


def canonical_edit(n: str) -> str:
    return EDITING_DICT.get(n, n)
